...
class BenchmarkForm(npyscreen.ActionForm):
    ...
    def create(self):
        ...
        self.wgcorr = self.add(npyscreen.TitleCombo, name="Type:", value=0, values=["Precision-Recall", "R-precision"], max_height=2)
        ...
        self.wqqueryid = self.add(npyscreen.TitleCombo, name="Query id:", value=0, values=range(1, 31), max_height=1)
    ...
    def while_editing(self):
        ...
        if os.path.isfile(self.wgexpectedfile.value) and os.path.isfile(self.wgqueryfile.value):
            ...
            self.wgquerytext.values = self.splitText(
                bench.load_query_text(int(self.wqqueryid.value) + 1),
    ...
    def on_ok(self):
        ...
        if os.path.isdir(self.wgworkingdir.value):
            ...
            if self.wgcorr.value == 0:
                bench.precisionRecallBenchmark(doc_index, model1, int(self.wqqueryid.value) + 1)
            else:
